...
Section::Section(Course course, Teacher& teacher, Semester semester, int year) : 
	_course{course}, _teacher{&teacher} ,_semester{semester}, year{year} {}
...
std::string Section::to_string() const {
	...
	return _course.to_string() + " for " + ::to_string(_semester) + " " 
	+ std::to_string(year) + " ("+ "taught by "+ _teacher->to_string() + ")";
}
